...
<html lang="es">
  ...
  <body>
    <div class="app">
      <aside class="sidebar"><div class="brand"><span class="brand-mark"></span><span>PINOLEROS.<b>AI</b></span></div><nav class="nav"><button class="active"><span class="nav-icon">✦</span>Diseña</button><button><span class="nav-icon">✚</span>Crea+</button><button><span class="nav-icon">▣</span>Mis diseños</button><p class="nav-title">EXPLORA</p><button><span class="nav-icon">♨</span>Tendencias</button><button><span class="nav-icon">◌</span>Estilos</button><button><span class="nav-icon">◇</span>Productos</button><button><span class="nav-icon">AI</span>Herramientas IA</button><button><span class="nav-icon">◉</span>Tutoriales</button></nav><div class="side-cta"><p>Convierte tus ideas<br>en productos reales</p><button>Ver servicios →</button></div></aside>
    ...
    <script>
      ...
      const auth=document.createElement('dialog');auth.innerHTML='<form class="studio-form" id="auth-form"><h2>Tu espacio PINOLEROS</h2><label>Correo<input name="email" type="email" required></label><label>Contraseña<input name="password" type="password" minlength="10" required></label><div class="status"></div><div class="form-actions"><button class="secondary" value="login">Iniciar sesión</button><button class="primary" value="register">Crear cuenta</button></div></form>';document.body.append(auth);
      const account=$('.avatar');account.addEventListener('click',()=>auth.showModal());auth.querySelector('form').addEventListener('submit',async e=>{e.preventDefault();const f=e.currentTarget,action=e.submitter.value,{r,x}=await j('/api/auth/'+action,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(Object.fromEntries(new FormData(f)))});if(!r.ok){f.querySelector('.status').textContent=x.error;return}account.textContent=x.user.email.slice(0,2).toUpperCase();auth.close()});
      ...
      const nav=[...document.querySelectorAll('.nav button')];nav[1].onclick=()=>$('#studio').showModal();nav[2].onclick=async()=>{const {r,x}=await j('/api/creations');if(!r.ok){auth.showModal();return}const gallery=document.createElement('dialog');gallery.innerHTML='<h2>Mis diseños</h2><div class="results">'+x.creations.map(c=>'<article class="result"><img src="'+c.imageUrl+'"><span>♥ '+c.title+(c.is_public?' · Público':' · Privado')+'</span></article>').join('')+'</div>';document.body.append(gallery);gallery.showModal()};nav.slice(3).forEach(b=>b.onclick=()=>document.querySelector('.section').scrollIntoView({behavior:'smooth'}));
      })();
    </script>
    <script>
