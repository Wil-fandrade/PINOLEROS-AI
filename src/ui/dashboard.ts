...
<html lang="es">
  ...
  <body>
    ...
    <script>
      ...
      const auth=document.createElement('dialog');auth.innerHTML='<form class="studio-form" id="auth-form"><h2>Tu espacio PINOLEROS</h2><label>Correo electrónico<input name="email" type="email" autocomplete="email" required></label><label>Contraseña<input name="password" type="password" autocomplete="current-password" minlength="10" required></label><label>Móvil <small>(requerido al crear cuenta)</small><input name="phone" type="tel" autocomplete="tel" inputmode="tel" placeholder="Ej. +505 8888 8888"></label><div class="status"></div><div class="form-actions"><button class="secondary" value="login">Iniciar sesión</button><button class="primary" value="register">Crear cuenta</button></div></form>';document.body.append(auth);
      const account=$('.avatar');account.addEventListener('click',()=>auth.showModal());auth.querySelector('form').addEventListener('submit',async e=>{e.preventDefault();const f=e.currentTarget,action=e.submitter.value;if(action==='register'&&!f.phone.value.trim()){f.querySelector('.status').textContent='Agrega tu número móvil para crear la cuenta.';return}const {r,x}=await j('/api/auth/'+action,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(Object.fromEntries(new FormData(f)))});if(!r.ok){f.querySelector('.status').textContent=x.error;return}account.textContent=x.user.email.slice(0,2).toUpperCase();auth.close();location.assign('/mi-espacio')});
