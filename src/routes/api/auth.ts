...
interface Credentials { email?: unknown; password?: unknown; }
...
async function credentials(request: Request): Promise<{ email: string; password: string } | null> {
  try {
    ...
    return email.includes("@") && email.length <= 254 && password.length >= 10 && password.length <= 128 ? { email, password } : null;
  } catch { return null; }
}
...
export async function register(request: Request, env: Env): Promise<Response> {
  ...
  await env.DB.prepare("INSERT INTO users (id, email, password_hash, password_salt) VALUES (?1, ?2, ?3, ?4)")
    .bind(id, input.email, password.hash, password.salt).run();
